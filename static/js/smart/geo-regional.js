...
import { apiSmartGet, setSmartLoader, showSmartSectionError, showSmartSectionEmpty,
...
export async function loadGeoSection(month, _retries) {
  ...
  try {
    ...
    if (d.computing) {
      if (_retries > 0) {
        setSmartLoader('geo', true);
        setTimeout(() => loadGeoSection(month, _retries - 1), 3000);
      } else {
        setSmartLoader('geo', false);
        showSmartSectionEmpty('geo', _t('Computation timed out'));
      }
      return;
    }
