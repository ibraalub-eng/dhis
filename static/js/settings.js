...
import { DataTable, scoreBadge, trendIcon, confidenceBar } from './table-utils.js';
        ...
        // ── Database Preview & Export ──────────────────────────
        window.previewDatabaseTables = function() {
            var container = document.getElementById('dbPreviewContainer');
            var content = document.getElementById('dbPreviewContent');
            if (!container || !content) return;
            container.style.display = 'block';
            content.innerHTML = '<p style="color:var(--text-muted);font-size:0.82rem;">Loading tables...</p>';

            fetch('/config/database/preview').then(function(r) {
                if (!r.ok) throw new Error('HTTP ' + r.status);
                return r.json();
            }).then(function(data) {
                if (!data.tables || !data.tables.length) {
                    content.innerHTML = '<p style="color:var(--text-muted);">No tables found.</p>';
                    return;
                }
                var html = '<div style="margin-bottom:0.6rem;font-size:0.82rem;color:var(--text-secondary);">' +
                    '<strong>' + data.total_tables + '</strong> tables found</div>';
                ...
                data.tables.forEach(function(table) {
                    ...
                    if (table.primary_keys && table.primary_keys.length) {
                        html += ' <span style="font-size:0.72rem;color:var(--accent-purple);">PK: ' + table.primary_keys.join(', ') + '</span>';
                    }
                    html += '</span>';
                    html += '<span style="color:var(--text-muted);font-size:0.75rem;">&#x25BC; click to expand</span>';
                    html += '</div>';
                    // Table body (hidden by default)
                    html += '<div class="db-table-body" style="display:none;padding:0.5rem;">';
                    // Columns list
                    html += '<div style="margin-bottom:0.4rem;font-size:0.78rem;color:var(--text-secondary);">';
                    html += '<strong>Columns:</strong> ' + table.columns.map(function(c) {
                        var isPK = table.primary_keys && table.primary_keys.indexOf(c) !== -1;
                        return '<code style="background:var(--bg-surface);padding:0.1rem 0.3rem;border-radius:3px;margin:0 0.15rem;' + (isPK ? 'color:var(--accent-purple);font-weight:600;' : '') + '">' + c + '</code>';
                    }).join(', ');
                    html += '</div>';
                    // Preview data table
                    if (table.preview && table.preview.length) {
                        html += '<div style="overflow-x:auto;"><table style="font-size:0.75rem;width:100%;border-collapse:collapse;">';
                        html += '<thead><tr>' + table.columns.map(function(c) {
                            return '<th style="padding:0.3rem 0.5rem;border-bottom:2px solid var(--border-default);text-align:left;font-weight:600;color:var(--text-primary);white-space:nowrap;">' + c + '</th>';
                        }).join('') + '</tr></thead><tbody>';
                        table.preview.forEach(function(row) {
                            html += '<tr>' + table.columns.map(function(c) {
                                var val = row[c];
                                ...
                                return '<td style="padding:0.25rem 0.5rem;border-bottom:1px solid var(--border-default);color:var(--text-secondary);max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="' + String(row[c] || '').replace(/"/g, '&quot;') + '">' + val + '</td>';
                            }).join('') + '</tr>';
                        });
                        html += '</tbody></table></div>';
                        if (table.row_count > 5) {
                            html += '<div style="font-size:0.72rem;color:var(--text-muted);margin-top:0.3rem;">Showing 5 of ' + table.row_count + ' rows</div>';
                        }
                    } else {
                        html += '<div style="font-size:0.78rem;color:var(--text-muted);padding:0.3rem;">No data in this table.</div>';
                    }
                    html += '</div></div>';
                });
                content.innerHTML = html;
            }).catch(function(err) {
                content.innerHTML = '<p style="color:var(--accent-red);">Error loading tables: ' + err.message + '</p>';
            });
        };

        window.exportDatabase = function() {
            var status = document.getElementById('dbExportStatus');
            if (status) status.textContent = 'Preparing export... (this may take a moment for large databases)';
            var btn = document.getElementById('btnExportDb');
            if (btn) btn.disabled = true;

            fetch('/config/database/export').then(function(r) {
                if (!r.ok) throw new Error('HTTP ' + r.status);
                // Extract filename from Content-Disposition header
                var disposition = r.headers.get('Content-Disposition') || '';
                var filenameMatch = disposition.match(/filename="?(.+?)"?$/);
                var filename = filenameMatch ? filenameMatch[1] : 'database_export.json';
                ...
                if (status) status.textContent = '✓ Export downloaded: ' + result.filename;
            }).catch(function(err) {
