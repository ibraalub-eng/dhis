                ...
                if (components.length) {
                    ...
                    components.forEach(function(c) {
                        ...
                        if (c.monthly && c.monthly.length) {
                            ...
                            // 2-column table: each month -> its missing indicators
                            var missingRows = c.monthly.filter(function(mm) { return mm.missing_indicators && mm.missing_indicators.length; });
                            if (missingRows.length) {
                                html += '<div style="font-size:0.75rem;font-weight:600;color:var(--text-secondary);margin:0.6rem 0 0.3rem;">' + __('Missing Indicators by Month') + '</div>';
                                html += '<div style="overflow-x:auto;">';
                                html += '<table style="width:100%;border-collapse:collapse;font-size:0.72rem;">';
                                html += '<thead><tr style="background:var(--bg-surface);">';
                                html += '<th style="text-align:left;padding:0.3rem 0.5rem;width:30%;">' + __('Month') + '</th>';
                                html += '<th style="text-align:left;padding:0.3rem 0.5rem;">' + __('Missing Indicators') + '</th>';
                                html += '</tr></thead><tbody>';
                                missingRows.forEach(function(mm) {
                                    html += '<tr style="border-bottom:1px solid var(--border-default);">';
                                    html += '<td style="padding:0.3rem 0.5rem;font-weight:500;white-space:nowrap;color:var(--accent-red);">' + mm.month + '</td>';
                                    html += '<td style="padding:0.3rem 0.5rem;">';
                                    html += '<div style="display:flex;flex-wrap:wrap;gap:3px;">';
                                    (mm.missing_indicators || []).forEach(function(mi) {
                                        html += '<span style="display:inline-block;background:rgba(198,40,40,0.12);color:var(--accent-red);padding:1px 6px;border-radius:4px;font-size:0.62rem;white-space:nowrap;">' + esc(mi) + '</span>';
                                    });
                                    html += '</div>';
                                    html += '</td>';
                                    html += '</tr>';
