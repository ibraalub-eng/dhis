        ...
        export async function apiGet(path, opts) {
            ...
            const promise = authFetch(API() + path).then(async res => {
        ...
        export async function apiPost(path, data) {
            const res = await authFetch(API() + path, { method: 'POST', body: data });
            if (!res.ok) throw new Error('HTTP ' + res.status + ': ' + await res.text());
            return res.json();
        }
        export async function apiPut(path, data) {
            const res = await authFetch(API() + path, { method: 'PUT', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(data) });
            if (!res.ok) throw new Error('HTTP ' + res.status + ': ' + await res.text());
            return res.json();
        }
        ...
        export async function apiPostJSON(path, data) {
            const res = await authFetch(API() + path, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(data) });
            if (!res.ok) throw new Error('HTTP ' + res.status + ': ' + await res.text());
            return res.json();
        }
